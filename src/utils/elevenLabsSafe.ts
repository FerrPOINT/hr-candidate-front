...
export class ElevenLabsSafe {
  ...
  constructor(conversation: unknown) {
  ...
  async startSession(config: SessionConfig): Promise<Session> {
    ...
    if (typeof (this.conversation as any).startSession === 'function') {
      try {
        const session = await (this.conversation as any).startSession(config);
  ...
  async sendMessage(text: string): Promise<void> {
    ...
    if (typeof (this.conversation as any).sendMessage === 'function') {
      try {
        await (this.conversation as any).sendMessage(text);
  ...
  async stopSession(): Promise<void> {
    ...
    if (typeof (this.conversation as any).stopSession === 'function') {
      try {
        await (this.conversation as any).stopSession();
  ...
  checkMethods(): { startSession: boolean; sendMessage: boolean; stopSession: boolean } {
    return {
      startSession: typeof (this.conversation as any)?.startSession === 'function',
      sendMessage: typeof (this.conversation as any)?.sendMessage === 'function',
      stopSession: typeof (this.conversation as any)?.stopSession === 'function'
    };
  }
  ...
  getAvailableMethods(): string[] {
    ...
    if (this.conversation) {
      Object.getOwnPropertyNames(this.conversation as object).forEach(name => {
        if (typeof (this.conversation as any)[name] === 'function') {
          methods.push(name);
        }
      });
    }
...
// Утилитарные функции
export const createElevenLabsSafe = (conversation: unknown): ElevenLabsSafe => {
  return new ElevenLabsSafe(conversation);
};

export const validateElevenLabsSDK = (conversation: unknown): boolean => {
