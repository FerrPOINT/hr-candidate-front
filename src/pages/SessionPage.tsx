...
// Безопасная функция для сохранения в localStorage
...
const SessionPage: React.FC = () => {
  ...
  // Сохраняем состояние чекбокса и данные формы при изменении с debounce
  useEffect(() => {
    const formData = { firstName, lastName, email, phone };
    const debouncedSaveForm = debounce((formData: any) => {
      saveToStorage('candidate_remember_me', rememberMe.toString());
      if (rememberMe && (formData.firstName || formData.lastName || formData.email || formData.phone)) {
        saveToStorage('candidate_form_data', formData);
      } else if (!rememberMe) {
        localStorage.removeItem('candidate_form_data');
      }
    }, 500);
    debouncedSaveForm(formData);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [firstName, lastName, email, phone, rememberMe]);

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setIsLoading(true);
    try {
      const data: CandidateAuthRequest = { firstName, lastName, email };
            ...
            <div className="space-y-4">
              ...
              <div>
                ...
                <input
                  ...
                  className="input-field bg-white/20 border-white/30 text-white placeholder-blue-200"
