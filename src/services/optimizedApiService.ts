...
import type { Position, Candidate, Interview, Question } from '../api/models';
...
function getFromSessionCache<T>(key: string): T | null {
  return (sessionCache.get(key) as T) || null;
}
...
class OptimizedApiService {
  ...
  async getVacanciesPageData(params?: Record<string, unknown>) {
    ...
    // Формируем результат
    const stats: Record<number, number | object | null> = {};
  ...
  async getDashboardPageData(params?: Record<string, unknown>) {
    console.log('🔄 Загружаем данные дашборда (batch запросы)');

    // Основные запросы
    const [positionsResponse, interviewsResponse] = await Promise.all([
      apiService.getPositions({ page: 1, size: 100 }),
      apiService.getInterviews({ page: 1, size: 20 })
    ]);
  ...
  async getInterviewsPageData(params?: Record<string, unknown>) {
